...
ok(main_conf['version'] == '3.1.1', 'configuração Tauri está na versão 3.1.1')
ok(re.search(r'^version = "3\.1\.1"$', cargo, re.M) is not None, 'pacote Rust está na versão 3.1.1')
...
ok('send_error(&mut stream' in sync_rs and 'O PC encerrou a transferência logo após validar a chave' in sync_rs, 'falhas pós-autenticação retornam diagnóstico em vez de cabeçalho vazio')
